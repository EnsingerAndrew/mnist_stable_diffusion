...
def get_pos_emb(positions, emb_dim):
    ...
    t_emb = positions[:, None].repeat(1, emb_dim // 2)
    t_emb = t_emb / factor
...
class UpBlock(nn.Module):
    ...
    def forward(self, x, out_down, t_emb, w_emb, guided):
        ...
        for i in range(self.num_layers):
            ...
            # EDIT IN_ATTN HERE 
            # N_img = in_attn.shape[1]
            # in_attn = torch.concat([in_attn, self.t_emb_layers[i](t_emb)[:, None, :]], dim=1)
            w_emb_i = self.w_emb_layers[i](w_emb)
            out_attn_self, _ = self.attentions[i](in_attn, in_attn, in_attn)
            out_attn_cross, _ = self.Xatts[i](in_attn, w_emb_i, w_emb_i)
            out_attn = out_attn_self + F.tanh(self.lamb_pre) * out_attn_cross * guided.view(-1, 1, 1)
...
class Unet(nn.Module):
    def __init__(self, WEMB):
        ...
        self.text_enc = nn.ModuleList([
            nn.TransformerEncoderLayer(self.w_emb_dim, 8, batch_first=True, dim_feedforward=256),
            nn.TransformerEncoderLayer(self.w_emb_dim, 8, batch_first=True, dim_feedforward=256),
            nn.TransformerEncoderLayer(self.w_emb_dim, 8, batch_first=True, dim_feedforward=256)          
        ])

        self.text_pos_enc = nn.Parameter(get_pos_emb(torch.as_tensor(range(0,self.max_tokens)), self.w_emb_dim))
    ...
    def forward(self, x, t, w, guided):
        ...
        num_tokens = torch.randint(3,self.max_tokens,(1,)).item()

        w_emb = self.word_embedding[w].unsqueeze(1).repeat(1,num_tokens,1)
        w_emb = w_emb + self.text_pos_enc[:num_tokens]

        for i in range(len(self.text_enc)): 
            w_emb = self.text_enc[i](w_emb)
...
if __name__ == "__main__": 
    ...
    WEMB = torch.load("embeddings.pth")
    ...
    count_parameters(mym.ups)
    count_parameters(mym.mids)
    count_parameters(mym.downs)
    count_parameters(mym.text_enc)
